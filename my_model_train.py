...
class ImageHelper():
  def __init__(self):
    ...
    self.files = self.files[3000:21000]
    random.shuffle(self.files)
    # self.files = self.files[:5000]
    self.i = 0

  def train_test_split(self, percentage=0.99, label='gender'):
...
def normal_full_layer(input_layer, size, name = None):
    input_size = int(input_layer.get_shape()[1])
    W = init_weights([input_size, size])
    b = init_bias([size])
    return tf.matmul(input_layer, W, name=name) + b

convo_1 = convolutional_layer(x,shape=[3,3,3,64])
convo_1 = convolutional_layer(convo_1,shape=[3,3,64,64])
convo_1_pooling = max_pool_2by2(convo_1)

convo_2 = convolutional_layer(convo_1_pooling,shape=[3,3,64,128])
convo_2 = convolutional_layer(convo_2,shape=[3,3,128,128])
convo_2_pooling = max_pool_2by2(convo_2)

convo_3 = convolutional_layer(convo_2_pooling,shape=[3,3,128,256])
convo_3 = convolutional_layer(convo_3,shape=[3,3,256,256])
convo_3 = convolutional_layer(convo_3,shape=[3,3,256,256])
convo_3_pooling = max_pool_2by2(convo_3)

# convo_4 = convolutional_layer(convo_3_pooling,shape=[3,3,256,512])
# convo_4 = convolutional_layer(convo_4,shape=[3,3,512,512])
# convo_4 = convolutional_layer(convo_4,shape=[3,3,512,512])
# convo_4_pooling = max_pool_2by2(convo_4)

convo_3_flat = tf.reshape(convo_3_pooling,[-1,8*8*256])

full_layer_one = tf.nn.relu(normal_full_layer(convo_3_flat,1024))

full_one_dropout = tf.nn.dropout(full_layer_one,keep_prob=hold_prob)

full_layer_three = tf.nn.relu(normal_full_layer(full_one_dropout,1024))

full_layer_four = tf.nn.relu(normal_full_layer(full_layer_three,512))

y_pred = normal_full_layer(full_layer_four,2,name='y_prediction')
...
with tf.Session(config = config) as sess:
  ...
  saver.save(sess, 'my_net/save_my_net.ckpt')